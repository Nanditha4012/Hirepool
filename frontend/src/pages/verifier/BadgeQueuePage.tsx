...
export default function BadgeQueuePage() {
  const [rows, setRows] = useState<BadgeQueueRow[]>([])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const [rowsResult, reasonsResult] = await Promise.all([listBadgeQueue(), listRejectionReasons('item')])
        if (cancelled) return
        setRows(rowsResult)
        setReasons(reasonsResult)
      } catch (err) {
        if (!cancelled) setError(err instanceof Error ? err.message : 'Failed to load the badge queue')
      } finally {
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-0.5 text-sm text-ink/70">
                        {row.platformName} · {row.badgeSelected} · {row.totalQuestionsSolved} solved
                      </p>
                      <a
