...
// The real verifier portal ships in Phases 2-5.
export default function VerifierStub() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const result = await apiFetch<PingResponse>('/verify/ping')
    ...
    <div className="mx-auto max-w-xl px-4 py-16 sm:px-6">
      <Card>
        <p className="mb-2 inline-block rounded-full bg-boost/10 px-2.5 py-0.5 text-xs font-semibold text-boost">
          Placeholder — real verifier portal ships in Phases 2-5
        </p>
        <h1 className="text-2xl font-bold text-ink">Verifier area</h1>
